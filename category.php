...
<div class="container" id="sales-products">
    ...
    <span style="margin-top: 70px;">Flashy Deals</span>
       <?php if (isset($_GET['id'])) { ?> 
                            <?php 
                             $titleid = $_GET['id'];
                             $title ="SELECT category_name, type_name FROM products INNER JOIN category ON category.category_id = products.category_id INNER JOIN jewelry_types ON jewelry_types.category_id = products.category_id WHERE types_id = $titleid GROUP BY type_name";
                             $titlequery = $db->query($title);
                            ?>
                             <?php while ($tt = mysqli_fetch_assoc($titlequery)) : ?>
                                  <h1 style="margin-right: 20px; margin-bottom: 0px;">
                                  <?php echo $tt['category_name'] ;?> : 
                                  </h1>
                                  <h4 style="margin-top: 0px; margin-left: 20px; color: #DD4B39; text-decoration: underline;">
            ...
            <div class="col-md-3">
                <span class="spantitle">Categories</span>
                 ...
                 <?php if (isset($_GET['id'])) { ?> 
                      <?php 
                       $jtype = $_GET['id'];
                          $tsql ="SELECT * FROM products WHERE type_id = $jtype";
